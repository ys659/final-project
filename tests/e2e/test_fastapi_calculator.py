...
def test_create_calculation_modulus(base_url: str):
    ...
    response = requests.post(url, json=payload, headers=headers)
    assert response.status_code == 201, f"Modulus calculation failed: {response.text}"
    data = response.json()
    assert data["result"] == 1, f"Expected 1, got {data.get('result')}"
...
def test_model_modulus():
    ...
    result = calc.get_result()
    assert result == 1, f"Modulus incorrect: expected 1, got {result}"
