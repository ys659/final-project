...
def test_modulus_get_result():
    """
    Test that Modulus.get_result returns correct remainder.
    Example: 10 % 3 = 1
    """
    from app.models.calculation import Modulus

    inputs = [10, 3]
    mod = Modulus(user_id=dummy_user_id(), inputs=inputs)
...
def test_modulus_by_zero():
    ...
    inputs = [10, 0]
    mod = Modulus(user_id=dummy_user_id(), inputs=inputs)
    with pytest.raises(ValueError, match="Cannot perform modulus operation with zero!"):
        mod.get_result()
...
def test_calculation_factory_modulus():
    ...
    assert isinstance(calc, Modulus), "Factory did not return Modulus instance."
    assert calc.get_result() == 2, f"Expected 2, got {calc.get_result()}"
...
def test_calculation_factory_invalid_type():
    ...
    with pytest.raises(ValueError, match="Unsupported calculation type"):
        Calculation.create(
            calculation_type='moduluss',  # unsupported type
            user_id=dummy_user_id(),
            inputs=[10, 3],
        )
...
def test_invalid_inputs_for_modulus():
    """
    Test modulus requires at least two numbers.
    """
    from app.models.calculation import Modulus

    mod = Modulus(user_id=dummy_user_id(), inputs=[10])
    with pytest.raises(ValueError, match="Inputs must be a list with at least two numbers"):
        mod.get_result()
